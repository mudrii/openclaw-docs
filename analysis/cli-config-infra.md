...
## v2026.2.24 Changes (Unreleased) <!-- v2026.2.24 -->

> **2 breaking changes in this release. Read the Breaking section carefully before upgrading.**

### Breaking <!-- v2026.2.24 -->

> **BREAKING — Control UI `allowedOrigins` required for non-loopback access**
> Non-loopback Control UI deployments now require `gateway.controlUi.allowedOrigins` to be set to an explicit list of full origins (e.g. `["https://mydomain.example.com"]`). If the list is absent, startup fails closed. To temporarily opt out while migrating, set `gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback=true` — this restores the previous host-header-derived behavior but is not safe for production.

> **BREAKING — `allowFrom` matching is now ID-only by default**
> Channel `allowFrom` entries are matched against stable IDs only. Name-based matching is no longer applied automatically. If your `allowFrom` lists contain display names or usernames rather than stable platform IDs, migrate them to stable IDs. To restore the previous behavior temporarily, set `channels.<channel>.dangerouslyAllowNameMatching=true`. (#24907)

### CLI / Doctor <!-- v2026.2.24 -->

- **Doctor/UX — suppress duplicate fix hint** — When `openclaw doctor --fix` is already in fix mode and produces no changes, the redundant "Run doctor --fix" suggestion is no longer emitted. (#24666)

- **Doctor/CLI — corrected stale recovery hints** — Stale recovery hint strings pointing to obsolete or nonexistent commands have been updated to use valid commands: `openclaw gateway status --deep` (for gateway probe failures) and `openclaw configure --section model` (for model-selection prompts). (#24485)

- **Doctor/Nix — skip Nix store symlink false positives** — State-integrity checks no longer emit permission warnings for paths that resolve into the Nix store (`/nix/store/...`). Symlinks in Nix store paths are read-only by design; flagging them as a permission problem was a false positive. (#24901)

- **Doctor/Memory** — `openclaw doctor` now queries the gateway-side default-agent memory embedding readiness directly instead of inferring it from generic gateway health. This surfaces accurate embedding status (index warm, model loaded, etc.) for memory-backed agents. (#22327)

- **Security audit — all-accounts scan** — `openclaw security audit` now iterates all configured accounts (not only the default account) when running detectors. The mutable-allowlist detectors used by `security audit` are now shared with `openclaw doctor` for consistent findings across both commands.

- **Auto-reply/Abort keywords — multilingual stop keywords** — The auto-reply abort shortcut recognizes stop keywords in additional languages: ES, FR, ZH, HI, AR, JP, DE, PT, RU. Trailing punctuation (`.`, `!`, `?`) is now stripped before keyword matching.

### Config <!-- v2026.2.24 -->

- **Config/Write — `unsetPaths` with immutable path-copy updates** — `unsetPaths` is applied using immutable path-copy semantics to avoid mutating shared config objects during write operations. `config get/set/unset` path traversal now rejects prototype-key segments and inherited-property traversal paths, closing a prototype-pollution vector. (#24134)

- **Config/Security — sensitive key redaction in `config.get` snapshots** — Dynamic catchall keys matching sensitive patterns (e.g. `env.*`, `skills.entries.*.env.*`) are redacted from `config.get` output snapshots. Round-trip restore behavior (export → import) is preserved: redacted keys carry a sentinel that prevents them from being written as literal redacted values.

- **Config/Write — prototype key blocks in account-id normalization** — Account-ID normalization code paths that write derived keys back to config now reject reserved prototype key names (`__proto__`, `constructor`, `prototype`) to prevent config-level prototype injection.

### Plugins <!-- v2026.2.24 -->

- **Plugin config entry keys use manifest `id`** — Plugin configuration entries are now keyed by the plugin manifest `id` field rather than the npm package name. This ensures plugin settings remain correctly bound when a package is renamed or aliased at install time. Existing installations are migrated automatically on first run. (#24796)

- **Legacy plugin schema fallback** — Plugins that expose a legacy schema without a `toJSONSchema()` method are no longer rejected during schema registration. A permissive object schema is generated as a fallback, maintaining backward compatibility with older plugin SDKs. (#24933)

### Update / Install <!-- v2026.2.24 -->

- **Update/Systemd — back up unit before overwrite** — The update flow now creates a backup of an existing systemd unit file before overwriting it. The backup is retained so the previous unit can be restored if the new unit is malformed. (#24350, #24937)

- **Install/Global detection — resolve symlinks for pnpm/bun paths** — When detecting a pnpm or bun global install, the installer now resolves symlinks in the candidate paths before comparison. This fixes false-negative global detection on systems where the global bin directory is a symlink. (#24744)

- **Infra/Windows — `dev=0` TOCTOU edge case** — Windows same-file identity checks (used to prevent TOCTOU race conditions) now correctly handle the `dev=0` edge case returned by some Windows filesystem drivers. (#24939)

### Sessions / Maintenance <!-- v2026.2.24 -->

- **`openclaw sessions cleanup` — per-agent targeting and disk-budget controls** — `sessions cleanup` gains per-agent store targeting: pass `--agent <id>` to scope cleanup to a single agent's store. New disk-budget config keys control eviction thresholds: <!-- v2026.2.24 -->

  | Config Key | Type | Description |
  |------------|------|-------------|
  | `session.maintenance.maxDiskBytes` | `number` | Hard ceiling; cleanup runs when exceeded |
  | `session.maintenance.highWaterBytes` | `number` | Soft ceiling; cleanup aims to reach this threshold |

  Transcript and archive cleanup logic is made safer (atomic moves, no partial deletes). Run-log retention behavior is now separate from transcript retention. (#24753)

- **Session store — canonicalize mixed-case session keys** — Inbound session keys (metadata and route-update paths) are normalized to lowercase on arrival. Legacy case-variant entries in existing session stores are migrated to their lowercase canonical form on first access to prevent duplicate-session creation from key-case mismatches. (#9561)

- **Auto-reply/Sessions — remove auth-key labels from `/new` and `/reset` messages** — The confirmation messages emitted by the `/new` and `/reset` session commands no longer include auth-key labels, which were being exposed unnecessarily to the channel. (#24384, #24409)

- **Subagents — configurable `sessions_spawn` timeout** — A new config key `agents.defaults.subagents.runTimeoutSeconds` sets the default wall-clock timeout for sessions spawned by `sessions_spawn`. Previously the timeout was hardcoded. Individual spawn calls can still override the value at call time. (#24594)

### Auth / Pairing <!-- v2026.2.24 -->

- **Pairing recovery hints — show `requestId` and explicit approval command** — When a gateway probe failure reports a `pairing-required` closure code, the CLI now emits the explicit approval command the operator should run (e.g. `openclaw nodes approve --request-id <id>`) including the `requestId` when it is safe to display in the terminal. (#24771)

- **Auth/OAuth — missing scopes classified as auth failure** — When an OAuth token is missing required scopes, the failure is now classified as an authentication failure (not a network or transient error). This enables clearer remediation messages and prevents retry loops. (#24761)

- **Onboarding/Custom providers — higher verification probe token budgets** — The token budget used by the verification probe during custom OpenAI-compatible and Anthropic-compatible provider onboarding is raised to accommodate models with large prompt overhead. Prevents false-negative "model unreachable" errors during onboarding. (#24743)
