...
- **CLI:** `openclaw status --usage` and `openclaw models list` show
  normalized provider quota windows (`X% left`, not per-response costs). Use
  `openclaw models auth list` for saved provider auth profiles.
