...
- `openclaw status --usage` and `openclaw models list` show provider **usage windows**
  (quota snapshots, not per-message costs). Use `openclaw models auth list`
  for saved provider auth profiles.
