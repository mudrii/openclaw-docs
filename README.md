...
**Current stable: v2026.2.23 (released 2026-02-24, validated against `main` through 2026-02-24) — Unreleased: post-v2026.2.23 (major security hardening + UX fixes, see Versioning below)**
...
| [CHANGELOG.md](CHANGELOG.md) | Consolidated changelog for v2026.2.14 → v2026.2.23 (released). Post-v2026.2.23 unreleased changes tracked in Versioning section below. |
...
- **UNRELEASED (post-v2026.2.23)** — Major security hardening pass. Breaking: Control UI non-loopback requires explicit `gateway.controlUi.allowedOrigins`; channel `allowFrom` is now ID-only by default (opt-back with `dangerouslyAllowNameMatching=true`). New: `agents.defaults.subagents.runTimeoutSeconds`, Kilo Gateway model list update, auto-reply/Abort multilingual stop keywords (ES/FR/ZH/HI/AR/JP/DE/PT/RU), CLI doctor recovery hint corrections (`openclaw gateway status --deep`, `openclaw configure --section model`). Security: 9 exec approval hardening entries (cross-node replay, busybox/toybox, env -S bypass, two-phase approval, safeBins validation, autoAllowSkills path collision, shell env /etc/shells-only), iOS deep link confirmation, session export XSS prevention, image tool workspace boundary, sandbox apply_patch workspaceOnly, commands allowFrom sender-only, config prototype pollution, Twilio voice replay, ACP permission auto-approval hardening. Channel fixes: WhatsApp (final-only payloads, dmScope routing, reasoning suppression), Discord (reasoning suppression, threading), Telegram (SSRF, reactions, polling, reasoning), Synology Chat (webhook deregistration), Web UI i18n. Provider/gateway fixes: OpenRouter reasoning.effort, Anthropic OAuth beta injection, Bedrock cache, Groq TPM, Gateway WS flood, Gateway restart, Config write immutability.
