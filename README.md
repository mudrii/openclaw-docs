...
**Latest published upstream release: v2026.3.28 (published 2026-03-29 UTC).**
**Current validated docs snapshot: v2026.3.28-1 (validated against `v2026.3.28`).**
...
- `v2026.3.28-1` (2026-03-29, docs release) — Rebased the docs snapshot to v2026.3.28. Major updates: xAI Responses API migration with plugin-owned `x_search` and auto-enable; MiniMax image-01 generation provider and M2.7-only catalog; async `requireApproval` in `before_tool_call` plugin hooks; ACP current-conversation binds for Discord, BlueBubbles, and iMessage; `apply_patch` enabled by default for OpenAI/Codex; bundled Claude CLI, Codex CLI, and Gemini CLI backends on plugin surface; Podman rootless simplification; `upload-file` action unification across Slack/Teams/Google Chat/BlueBubbles; Matrix TTS native voice bubbles; `openclaw config schema` CLI command; Config/TTS auto-migration; Gateway-backed channel MCP bridge; Memory/QMD CJK chunking and slugified path resolution; LINE timing-safe HMAC; extended web search key audit; and comprehensive agent, channel, and memory reliability fixes.
...
- **7,627 TypeScript files** analyzed (`src/`, `extensions/`, `ui/`, `test/`, `scripts/` — `.ts` + `.tsx`; validated against `v2026.3.28`)
