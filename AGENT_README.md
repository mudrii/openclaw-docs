...
> **Current docs version: v2026.5.2 (2026-05-04).** Latest published upstream release: v2026.5.2 (signed tag, published 2026-05-02 23:37:55 UTC).
...
**v2026.5.2 additions (current release line):**

- **Current release-line checks:** this docs snapshot is aligned to upstream `v2026.5.2` (signed tag, published 2026-05-02 23:37:55 UTC; window `v2026.4.26..v2026.5.2`, 3,289 commits). Include npm-first official plugin cutover and ClawHub artifact metadata, `openclaw plugins list --json` dependency state, `git:` plugin installs, scoped plugin runtime preloads, descriptor-first plugin tool planning, restrictive tool profiles no longer widened by configured `tools.exec`/`tools.fs` without explicit `tools.alsoAllow`, SDK artifact list/get/download RPCs, active-run steering queue defaults, `messages.visibleReplies`, NVIDIA provider onboarding/catalogs, people-aware Memory Wiki and Active Memory per-conversation filters, `openclaw gateway restart --force`/`--wait`, `openclaw proxy validate`, `OPENCLAW_INCLUDE_ROOTS` for approved `$include` roots, `threadBindings.spawnSessions`, WhatsApp Channel/Newsletter `@newsletter` outbound targets, reusable Discord access groups, Slack App Home defaults and thread tracking, Google Meet `end-active-conference` and `test-listen`, xAI Grok 4.3 default catalog entry, OpenAI-compatible TTS `extraBody`, native Codex runtime documentation for `openai/gpt-*` plus `agentRuntime.id: "codex"`, `agents.defaults.skipOptionalBootstrapFiles`, `heartbeat_respond`, SDK-facing `tools.invoke`, Control UI/WebChat resilience updates, session/task/gateway hot-path performance work, and broad Telegram/Discord/Slack/Signal/WhatsApp/provider/media correctness fixes.
- **Behavioral change surface (`v2026.4.26..v2026.5.2`):** verify plugin install/update records and npm-vs-ClawHub source selection; gateway restart deferral/force semantics; approved include-root confinement; thread-bound spawn migration from legacy keys; Codex native runtime routing (`openai/gpt-*` with `agentRuntime.id: "codex"` vs `openai-codex/*` PI OAuth); Discord access-group authorization; WhatsApp newsletter target routing; Slack thread continuation after Gateway restart; session lock timeout behavior; plugin tool descriptor caching and optional-tool visibility; proxy validation behavior; and Control UI/WebChat reconnect/stop recovery.

**v2026.4.26 additions (historical):**

- **Current release-line checks:** this docs snapshot is aligned to upstream `v2026.4.26` (signed tag, published 2026-04-28 UTC; window `v2026.4.25..v2026.4.26`, ~831 commits). Include QQBot full group-chat support with C2C `stream_messages` streaming and unified `sendMedia`, Tencent Yuanbao external channel (`openclaw-plugin-yuanbao`, community plugin), Cerebras as bundled first-party provider (`extensions/cerebras/`), Google Live browser Talk realtime sessions and gateway relay for backend-only realtime voice plugins, plugin-manifest takeover of provider routing tables (model-id normalization, endpoint host metadata, OpenAI-compatible request-family hints, alias/suppression catalog rules) so adding a provider becomes manifest-only, transactional plugin config helpers replacing direct load/write, layered runtime-dependency roots for `OPENCLAW_PLUGIN_STAGE_DIR`, Memory/OpenAI-compatible asymmetric embedding `inputType`/`queryInputType`/`documentInputType` controls, Ollama model-specific retrieval-query prefixes for `nomic-embed-text`/`qwen3-embedding`/`mxbai-embed-large`, raw config pending-changes diff panel with redaction in Control UI, `openclaw matrix encryption setup` single-shot E2EE bootstrap, `agents.defaults.compaction.maxActiveTranscriptBytes` opt-in JSONL preflight trigger, `openclaw migrate` CLI subcommand with Claude Code/Desktop and Hermes importers, ACP Claude adapter ship-with-OpenClaw and `acp.dispatch.enabled=false` no longer blocks explicit `runtime="acp"` bootstrap, `openclaw nodes remove`, validated `--wrapper`/`OPENCLAW_WRAPPER` LaunchAgent/systemd install path, top-level `hostname`/`agent_id`/`session_id`/`channel` JSONL log fields with W3C `traceparent` propagation through Gateway requests/spans/headers, OTEL privacy-safe model-call payload byte/latency capture, durable Windows-startup gateway restart after `openclaw update`, Bonjour mDNS DNS-safe hostname default, browser CDP launch circuit-break per profile, exec approval-set `source: "allow-always"`/`commandText` allowlist metadata, host-local-timezone session-memory hooks, and `gateway.health` runtime-backed channel/account state.
- **Behavioral change surface (`v2026.4.25..v2026.4.26`):** verify QQBot group activation modes and FIFO message queue with deliver debounce; provider manifest catalog metadata loading at startup (no more bundled-provider routing tables in core); transactional plugin config mutation API with explicit restart-follow-up policy; `OPENCLAW_PLUGIN_STAGE_DIR` layered roots resolution order; `agents.subagents.allowAgents` enforcement on explicit `sessions_spawn(agentId=...)` self-targets (no longer auto-allowed); `acp.dispatch.enabled=false` paired with explicit `sessions_spawn(runtime="acp")` (now allowed); local OpenAI-compatible providers with only `baseUrl` defaulting to Chat Completions and trusting loopback model requests; Discord/Slack/Mattermost `user:`/`channel:` target syntax; `agents.defaults.compaction.maxActiveTranscriptBytes` rotation semantics; `cron.maxConcurrentRuns` nested isolated-agent lane; cron isolated-execution timeout starting only inside isolated lane; `openclaw browser doctor`/CDP per-profile circuit breakers when changing browser/launch code paths; provider-native reasoning effort declarations in Groq/LM Studio (no longer OpenAI-only `low`/`medium`); Anthropic Opus 4.7/Sonnet 4.6 trailing assistant prefill stripping when extended thinking is enabled; `dreaming.model` knob for narrative subagents; transcript-level redaction for persisted session text and escaped character classes in safe redaction regexes; and `gateway.proxy` `ALL_PROXY`/`all_proxy` plumbing through Undici env-proxy dispatcher and provider proxy-fetch helper.
- **Removed in v2026.4.26 (BREAKING):** `extensions/coven/` deleted; release config baseline refreshed. Coven now lives behind an opt-in default-off ACP runtime bridge — operators must remove all `extensions/coven/` references from configs and explicitly opt in via the ACP runtime bridge to keep Coven workflows. Refresh config validation expectations and any cached `extensions/coven/` plugin paths in operator setup paths.

**v2026.4.25 additions (historical):**

- **Release-line sync (`v2026.4.25`):** covered v2026.4.22, v2026.4.23, and v2026.4.25 windows. Include cold plugin registry as default startup path, dreaming decoupled from heartbeat, `/models add` live-only runtime registration, `sessions_spawn` forked context opt-in, `config set` mode flags, OpenAI Codex onboarding changes, and GenAI OTel semantics.
