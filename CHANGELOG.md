...
- **Security/tool policy (`v2026.4.29`):** configured tool sections such as `tools.exec` and `tools.fs` no longer implicitly widen restrictive profiles like `messaging` or `minimal`; users must add explicit `tools.alsoAllow` entries and startup warnings identify affected configs.
...
- **Gateway/SDK artifacts (`v2026.4.29`):** SDK-facing artifact list/get/download RPCs and App SDK helpers add transcript provenance and download-source guardrails.
...
- **Messages/queue (`v2026.4.29`):** active-run queueing defaults to steering behavior with follow-up fallback debounce, `messages.visibleReplies` lets operators require visible output to go through `message(action=send)`, and subagent chat/broadcast payloads expose `spawnedBy`.
...
- **Providers/NVIDIA (`v2026.4.29`):** NVIDIA hosted models gain API-key onboarding, setup docs, static catalog metadata, and literal provider-prefixed model refs.
...
- **Memory/wiki (`v2026.4.29`):** Memory Wiki adds people metadata, aliases, person cards, relationship graphs, privacy/provenance reports, evidence drilldown, and person/source search modes; Active Memory adds per-conversation allow/deny filters and bounded partial recall summaries on timeout.
...
- **CLI/models (`v2026.4.26`):** route provider-filtered model listing through an explicit source plan (user config → installed manifest rows → Provider Index previews → scoped runtime fallbacks) so authority order stays stable without an additional catalog cache.
...
- **Matrix/E2EE (`v2026.4.26`):** new `openclaw matrix encryption setup` command — enable Matrix encryption, bootstrap recovery, and print verification status from a single setup flow.
...
- **Coven extension removed (BREAKING) (`v2026.4.26`):** `extensions/coven/` deleted; release config baseline refreshed. Coven now lives behind an opt-in ACP runtime bridge (default-off, explicit operator opt-in) — remove all `extensions/coven/` references from operator configs.
