...
## v2026.4.5 Behavioral Changes

- **Legacy public config aliases are no longer the documented surface:** old public keys such as `talk.voiceId`, `talk.apiKey`, `agents.*.sandbox.perSession`, `browser.ssrfPolicy.allowPrivateNetwork`, and channel/group/room `allow` toggles should be treated as migration-only compatibility. Canonical paths plus `enabled` are now the release-line contract, with `openclaw doctor --fix` as the supported migration path.
- **Released memory runtime is split across `src/memory-host-sdk/` and `extensions/memory-core/src/memory/`:** dreaming, embeddings, and host/runtime helpers live under `src/memory-host-sdk/`; QMD, search managers, and sync/index orchestration live under `extensions/memory-core/src/memory/`. Older docs that point at a standalone `src/memory/` tree are historical.
- **Media generation is first-class:** `music_generate` and `video_generate` are now built-in tools, with released provider surfaces under `src/music-generation/`, `src/video-generation/`, `src/media-generation/`, and bundled workflow support in `extensions/comfy/`.
- **OpenAI/GPT reply delivery no longer leaks planning text:** commentary is buffered until `final_answer`, GPT-5/Codex defaults are shorter and lower-verbosity, and planning-only turns get a one-shot retry before the run is treated as complete.
- **Per-channel `contextVisibility` now affects routing context:** channels can filter quoted/thread/fetched context by allowlist instead of always forwarding it verbatim. If you touch channel context assembly, test allowlist and quote-filtered paths explicitly.
- **APNs and Matrix native exec approvals are part of the released approval surface:** approval UX is no longer just Web UI / Slack / Discord / Telegram. Changes to approval lifecycle must account for iOS modal delivery and Matrix-native room/thread flows.
- **Bundled CLI text-provider backends are gone from the public surface:** `agents.defaults.cliBackends` and the old bundled CLI text-provider path are removed; ACP harness sessions and native bundled providers remain supported.
- **ACPX is embedded and `reply_dispatch` is generic:** bundled ACPX no longer depends on an extra external CLI hop, and reply interception can now be owned by bundled plugins through the generic `reply_dispatch` hook.
