...
### v2026.5.4
- **`openclaw models auth list [--provider <id>] [--json]`:** new CLI command to inspect saved per-agent auth profiles. Use near other `openclaw models` commands.
- **`agents.defaults.toolProgressDetail: "raw"`:** new config key that overrides verbose mode tool output globally. Set in `agents.defaults` to force raw tool progress detail across all agents.
- **`registerIfAbsent` plugin keyed-store API:** new Plugin SDK primitive for atomic keyed-store dedupe claims; use instead of manual read-check-write when registering plugin-owned runtime state entries.
- **`SessionEntry` slot projection and scoped trusted-policy session extension reads:** Plugin SDK now exposes `SessionEntry` slot projection so plugins can read scoped session fields without loading the full session entry.
- **`before_agent_finalize` bounded retry hook:** new Plugin SDK hook for workflow plugins to attach bounded retry instructions before agent turn finalization.
- **Per-runtime sandbox registry shards:** sandbox container/browser registry entries are now per-runtime shard files. `openclaw doctor --fix` migrates legacy monolithic registry files.
- **Gateway startup deferred non-readiness sidecars:** non-readiness sidecars now defer until after the ready signal, improving cold-start performance.
- **Post-compaction tool-loop guard:** `tools.loopDetection.postCompactionGuard.windowSize` (default 3) aborts runs sending the same `(tool, args, result)` triple after auto-compaction-retry. Disable via `tools.loopDetection.enabled`.
